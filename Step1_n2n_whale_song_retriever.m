% Whale Song Retriever (Auto-restart, parallel version)
...
% Optimized function to read only necessary portions of the audio file.
...
% Extract file start time from filename
...
for attempt = 1:max_attempts
    try
        audioInfo = audioinfo(read_fileName);
        Fs = audioInfo.SampleRate;
        numSamples = audioInfo.TotalSamples;
        wav_duration = audioInfo.Duration;
        break; % Exit the loop if successful
    catch
        if attempt < max_attempts
            warning('Failed to read audio info for detection %d. Retrying in %d seconds... (Attempt %d of %d)', ...
                iteration, retry_delay, attempt, max_attempts);
            disp(read_fileName)
            pause(retry_delay);
        else
            warning('Failed to read audio info for detection %d after %d attempts. Skipping.', ...
...
% Convert ROI_datestring to datetime
detection_datetime = datetime(char(ROI_datestring), 'InputFormat', wav_dateformat_serial);

% Compute time difference between detection and file start
detection_time_rel = seconds(detection_datetime - file_start_datetime); % in seconds

% Compute sample index for detection time
detection_sample_index = round(detection_time_rel * Fs);

% Compute ROI start and end sample indices
ROIstart = detection_sample_index - round(buffer_duration * Fs);
ROIend = detection_sample_index + round((call_duration + buffer_duration) * Fs);

% Ensure ROIstart >= 1
ROIstart = max(1, ROIstart);

% Initialize detectionAudio
detectionAudio = [];

% Read the required samples from the current file
if ROIstart <= numSamples
    current_ROIend = min(ROIend, numSamples);
    for attempt = 1:max_attempts
        try
            [audiodata, ~] = audioread(read_fileName, [ROIstart, current_ROIend]);
            detectionAudio = [detectionAudio; audiodata];
            break; % Exit the loop if successful
        catch
            if attempt < max_attempts
...
% If ROIend > numSamples, we need to read from the next file
if ROIend > numSamples
    ...
    if current_file_index < length(wavs_filelist)
        ...
        next_ROIstart = 1;
        next_ROIend = ROIend - numSamples;

        % Read required samples from next file
        for attempt = 1:max_attempts
            try
                [next_audiodata, ~] = audioread(next_filepath, [next_ROIstart, next_ROIend]);
                detectionAudio = [detectionAudio; next_audiodata];
                break; % Exit the loop if successful
            catch
                if attempt < max_attempts
                    warning('Failed to read the next audio file for detection %d. Retrying in %d seconds... (Attempt %d of %d)', ...
                        iteration, retry_delay, attempt, max_attempts);
                    disp(next_filepath)
                    pause(retry_delay);
                else
                    warning('Failed to read the next audio file for detection %d after %d attempts. Skipping.', ...
                        iteration, max_attempts);
                    successFlag = false;
                    return;
                end
...
% Adjust the length of detectionAudio
if numel(detectionAudio) > expected_num_samples
    detectionAudio = detectionAudio(1:expected_num_samples);
elseif numel(detectionAudio) < expected_num_samples
    detectionAudio = [detectionAudio; zeros(expected_num_samples - numel(detectionAudio), 1)];
end
